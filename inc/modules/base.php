...
if ( ! class_exists( 'Fashion_daily_Module_Base' ) ) {

	/**
	 * Define Fashion_daily_Module_Base class
	 */
	abstract class Fashion_daily_Module_Base {
		...
		public function load_child_modules( $childs = array() ) {
			...
			foreach ( $childs as $child => $childs_of_child ) {
				fashion_daily_theme()->load_module( $child, $childs_of_child );
			}
